...
app.post('/api/contact', strictLimiter, asyncRoute(async (req, res) => {
  ...
  try {
    await sendMail({
      to: email,
      subject: `We received your message — AppAholic`,
      html: wrapEmail({
        preheader: 'Thanks for reaching out — we reply within one business day.',
        title: `Thanks, ${esc(String(name).split(' ')[0])} 🎉`,
        bodyHtml: `<p>We've received your message and typically reply within <strong>one business day</strong>.</p>
          <table width="100%" style="margin:18px 0;border-collapse:collapse;font-size:13px;">
            <tr><td style="padding:7px 0;color:#6B6B85;">Topic</td><td style="padding:7px 0;text-align:right;font-weight:600;">${esc(topicLabel)}</td></tr>
          </table>
          <p style="color:#6B6B85;font-size:13px;border-top:1px solid #E4E4F0;padding-top:12px;">Your message:<br/>${esc(message)}</p>`,
        ctaText: 'Browse Apps', ctaUrl: SITE_URL,
      }),
    });
    await sendMail({
      ...
      html: wrapEmail({
        ...
        bodyHtml: `
          <table width="100%" style="border-collapse:collapse;font-size:13px;">
            <tr><td style="padding:5px 0;color:#6B6B85;width:110px;">From</td><td style="padding:5px 0;font-weight:500;">${esc(name)} (${esc(email)})</td></tr>
            <tr><td style="padding:5px 0;color:#6B6B85;">Topic</td><td style="padding:5px 0;font-weight:500;">${esc(topicLabel)}</td></tr>
          </table>
          <p style="margin-top:14px;white-space:pre-wrap;">${esc(message)}</p>`,
      }),
    });
    res.json({ ok: true });
  } catch (err) {
